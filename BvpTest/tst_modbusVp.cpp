...
public:
    ...
    TModbusVp_Test() { mParam = TParam::getInstance(); }
...
protected:
    ...
    void TearDown() override { delete mModbus; }
};
...
TEST_F(TModbusVp_Test, netAddress)
{
    ...
    for (uint16_t address = 0; address <= 1000; address++)
    {
        ...
        ASSERT_EQ(check, mModbus->setNetAddress(address)) << " wrong address is " << address;

        if (check)
        {
            ASSERT_EQ(address, mModbus->mNetAddress) << " wrong address is " << address;
            last = address;
        }
        else
        {
            ASSERT_EQ(last, mModbus->mNetAddress) << " wrong address is " << address;
        }
    }
}
...
TEST_F(TModbusVp_Test, isBlockPrm)
{
    ...
    for (uint16_t i = 0; i <= (DISABLE_PRM_MAX + 1); i++)
    {
        ...
        ASSERT_EQ(i == DISABLE_PRM_disable, mModbus->isBlockPrm()) << " wrong value is " << i;
    }
}
...
{
    QVector<param_t> list = { PARAM_comPrmBlk08to01,
...
TEST_F(TModbusVp_Test, getSwitchLed_One)
{
    ...
    for (uint16_t i = PARAM_comPrmBlk08to01; i <= PARAM_comPrdBlk24to17; i++)
    {
        ...
        ASSERT_EQ(valueEn, mModbus->getSwitchLed(param, ON_OFF_off)) << " params is " << param;

        uint8_t valueDs = i & 0xFF;
        ASSERT_EQ(valueDs, mModbus->getSwitchLed(param, ON_OFF_on)) << " params is " << param;
    }
