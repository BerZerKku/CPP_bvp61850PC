...
protected:
    void SetUp() override { mAlarm = new BVP::TExtAlarm(); }

    void TearDown() override { delete mAlarm; }
};
...
TEST_F(TExtAlarm_Test, config)
{
    for (uint8_t i = 0; i < EXT_ALARM_MAX; i++)
    {
        ...
        TExtAlarm::resetMode_t rmode =
            (i == EXT_ALARM_fault) ? TExtAlarm::RESET_MODE_off : TExtAlarm::RESET_MODE_direct;
...
TEST_F(TExtAlarm_Test, reset)
{
    ...
    for (uint8_t i = 0; i < EXT_ALARM_MAX; i++)
    {
        ASSERT_FALSE(mAlarm->getAlarmOutputSignal(extAlarm_t(i))) << "signal " << extAlarm_t(i);
    }

    // Все входные сигналы true
    for (uint8_t i = 0; i < EXT_ALARM_MAX; i++)
    {
        ASSERT_TRUE(mAlarm->setAlarmInputSignal(extAlarm_t(i), true)) << "signal " << extAlarm_t(i);
    }
    ...
    for (uint8_t i = 0; i < EXT_ALARM_MAX; i++)
    {
        ASSERT_TRUE(mAlarm->getAlarmOutputSignal(extAlarm_t(i))) << "signal " << extAlarm_t(i);
    }
    ...
    for (uint8_t i = 0; i < EXT_ALARM_MAX; i++)
    {
        ...
        result = false;
        ASSERT_EQ(false, mAlarm->getAlarmOutputSignal(extAlarm_t(i))) << "signal " << extAlarm_t(i);
    }
...
TEST_F(TExtAlarm_Test, alarmSignal_default)
{
    ...
    for (uint8_t i = 0; i < EXT_ALARM_MAX; i++)
    {
        ...
        ASSERT_EQ(true, mAlarm->getAlarmOutputSignal(extAlarm_t(i))) << "signal " << extAlarm_t(i);
