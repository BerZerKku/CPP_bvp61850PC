...
//
vpReg::vpReg(QWidget *parent) : QWidget(parent), ui(new Ui::vpReg)
{
    ui->setupUi(this);
...
//
void vpReg::setEnLeds(quint16 value)
{
    ui->sbRegEnable->setValue(value);
}

//
void vpReg::setDsLeds(quint16 value)
{
    ui->sbRegDisable->setValue(value);
}

//
void vpReg::setBtns(quint16 value)
{
...
//
void vpReg::setRegime(vpItem::regime_t regime)
{
    for (vpItem &bit : bits)
    {
...
//
void vpReg::pressedBtnSlot(int bit, bool checked)
{
    qDebug() << "bit = " << bit << ", checked = " << checked;
}

void vpReg::changedEnLeds(int value)
{
    for (size_t index = 0; index < bits.size(); index++)
    {
...
void vpReg::changedDsLeds(int value)
{
    for (size_t index = 0; index < bits.size(); index++)
    {
...
void vpReg::changedBtns(int value)
{
    for (size_t index = 0; index < bits.size(); index++)
    {
