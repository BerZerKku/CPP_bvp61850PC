...
TSerial::~TSerial()
{
    // Периодически при закрытии приложения выскакивает ошибка
...
bool TSerial::setParityList(QVector<QSerialPort::Parity> &values)
{
    ...
    parityString.insert(QSerialPort::NoParity,   "  РќРµС‚ (N)");
    parityString.insert(QSerialPort::EvenParity, "  Р§РµС‚ (E)");
    parityString.insert(QSerialPort::OddParity,  "РќРµС‡РµС‚ (O)");
