...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(&timer100ms, &QTimer::timeout, ui->paramTree, &TParamTree::updateParameters);
...
void MainWindow::initSerial(TSerial* serial, MainWindow::serialCfg_t* cfg)
{
    ...
    connect(serial, &TSerial::sendFinished, [=]() { cfg->protocol->sendFinished(); });

    connect(serial, &TSerial::openPort, this, [=]() { serialStart(serial); });
    connect(serial, &TSerial::closePort, this, [=]() { serialStop(serial); });
}
...
void MainWindow::alarmLoop()
{
    ...
    for (uint8_t i = 0; i < EXT_ALARM_MAX; i++)
    {
        ...
        {
            uval32 = mParam->getValue(PARAM_blkComPrmAll, SRC_int, ok);
            if (!ok)
            {
