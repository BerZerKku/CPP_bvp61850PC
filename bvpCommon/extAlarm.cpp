...
const TExtAlarm::signal_t TExtAlarm::mSignal[] = { { .signal     = EXT_ALARM_model61850,
...
bool TExtAlarm::isSignalForDeviceReset()
{
    constexpr uint16_t mask =
        (1 << EXT_ALARM_channelFault) | (1 << EXT_ALARM_warning) | (1 << EXT_ALARM_fault);
...
void TExtAlarm::setSignal(extAlarm_t signal, bool value)
{
    ...
    if (signal < EXT_ALARM_MAX)
    {
        ...
        else
        {
            ...
            {
                if ((mAlarmReset != ALARM_RESET_manual) && (s->alarmReset != ALARM_RESET_manual))
                {
                    mAlarmOut = setBitValue(mAlarmOut, signal, value);
                }
            }
